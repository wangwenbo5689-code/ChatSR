...
class RagQueryExpander:
    ...
    def __init__(self, generate_summary: Callable[..., str]):
        """初始化查询扩展器
        ...
        self.history_manager = RagHistoryManager(stream_generate_answer=self._stream_summary)

    def _stream_summary(self, *, history=None, max_new_tokens=256, temperature=0.2, **_kwargs):
        prompt = ""
    ...
    def _prepare_history_context(
        ...
        keep_turns = self.history_manager.RECENT_KEEP_TURNS
        ...
        summary_turns = older_history[-self.history_manager.RECENT_SUMMARY_TURNS:]
        ...
        try:
            generated = (
                self.generate_summary(prompt, max_new_tokens=256, temperature=0.2) or ""
            ).strip()
            if generated:
                summary = generated
    ...
    def _optimize_query(self, query: str, history_context: str, should_optimize: bool) -> str:
        ...
        try:
            optimized_raw = (
                self.generate_summary(optimize_prompt, max_new_tokens=120, temperature=0.2) or ""
            ).strip()
    ...
    def expand(
        ...
        try:
            expansion_result = self.generate_summary(prompt, max_new_tokens=200, temperature=0.3)
    ...
    def classify_intent(self, query: str) -> str:
        ...
        try:
            result = (self.generate_summary(prompt, max_new_tokens=20, temperature=0.0) or "").strip()
        except Exception as exc:
